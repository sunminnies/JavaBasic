...
public class ByteStream {
    public static void main(String[] args) {
        ...
        // 파일을 바이트 단위로 입출력하려면 InputStream 또는 OutputStream 클래스를 사용함

        // 바이트 기반으로 스트림을 읽거나 씀 : 바이트 기반(1byte)
        // 주로 바이너리 기반 데이터(이미지, 동영상)를 처리할 때 사용
        ...
        try {
            ...
            // 바이트 기반 스트림에서 문자를 저장하려면 OutputStreamWriter 또는 PrintWriter 클래스를 사용해야함
        ...
        try {
            ...
            // 한 바이트씩 읽어 문자로 적절히 변환한 후 result 변수에 담아 한꺼번에 출력함
            while ((ch = isr.read()) != -1) {
                result += (char)ch;
            }
        ...
        try {
            ...
            byte readData[] = new byte[512]; // 버퍼를 활용해 한번에 읽어들일 바이트 크기 지정
            bis.read(readData); // bis를 이용해서 512바이트의 데이터를 읽어서 readData 변수에 저장
