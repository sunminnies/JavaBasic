...
public class JavaString {
    public static void main(String[] args) {
        ...
        // 문자열을 나타내기 위해 c언어에서는 char 배열로 표현하지만 자바에서는 String 이라는 클래스를 사용함
        // String 객체는 한번 생성되면 그 값을 읽기만 가능, 수정은 불가
        // => 불변객체 immutable object
        // String 객체값 비교시 equals 메서드를 사용해야함
        // charAt, concat, indexOf, trim, length, toLowerCase, toUpperCase, split, substring

        // 자바에서 String 객체는 메모리 재활용interning함
        // 즉, 생성된 String 객체값을 String Pool에 저장하는데, 그 다음에 생성한 String 객체값이
        // 이전과 유사한 경우 String Pool에 이미 만들어둔 객체를 재활용함
        String str1 = "Hello, World!";  // 힙 영역 내 상수풀에 생성
        String str2 = new String("Hello, World!");  // 힙 영역에 생성
        String str3 = "Hello, World!";  // 힙 영역 내 상수풀에 생성

        // 문자열 비교시 == 연산자는 값의 동등을 비교하는 것이 아니고 값이 저장된 위치를 비교함 (참조값 비교)
        if (str1 == str2) System.out.println("str1과 str2의 저장 위치가 같음");
        if (str1 == str3) System.out.println("str1과 str3의 저장 위치가 같음");

        // 문자열 비교시 equals 메서드를 사용하면 변수에 저장된 값을 비교함 (실제값 비교)
        if (str1.equals(str2)) System.out.println("str1과 str2는 값이 같음");
        if (str1.equals(str3)) System.out.println("str1과 str3는 값이 같음");
        ...
        // 문자열에 + 연산자를 사용하면 StringBuilder로 변환한 후 Append로 문자열을 더하고 toString으로 문자열로 변환
        ...
        // 문자열의 실제 바이트수를 알아내려면 getByte를 사용해야함
        ...
        // ex) hello, java!!라는 문자열에서 각 단어의 첫글자를 대문자로 만드는 코드를 작성해보세요 (capitalization)
        String l = "hello, java!!";
        ...
        // ex) hello, java!!라는 문자열에서 각 단어의 첫글자만 골라서 대문자로 만드는 코드를 작성해보세요 (capitalization)
        ...
        // ex) 주민등록번호에서 생년월일을 추출해봄
        // 123456-1234567
        System.out.println(jumin.substring(0,6));   // 0~5까지 문자추출

        // ex) 주민등록번호에서 생년월일을 제외한 나머지를 추출해봄
        // System.out.println(jumin.substring(7,14));
        System.out.println(jumin.substring(7)); // 끝위치를 생략하면 시작 위치를 기준으로 모든 글자를 출력해줌
        ...
        // format : c언어의 printf처럼 서식문자열을 이용해서 변수들과 문자열을 조합해서 문자열을 생성하는 메서드
        ...
        // %tM : Minute
        // %tS : second
        String fmt = "%tY년 %<tm년 %<td일 %<tH시 %<tM분 %<tS초 \n";
        // sunmin_today 변수 하나로 날짜와 시간을 다 출력하기 위해 < 기호를 사용함
       String result = String.format(fmt, today);
        System.out.println(result);

        // StringBuilder, StringBuffer
        // String 객체는 불변객체이므로 새로운 문자열이 추가되면 기존 공간 대신 새로운 공간을 생성한 후 추가함 -> 메모리 낭비 심함
        // StringBuilder나 StringBuffer를 이용하면 기존 공간에 새로운 문자열을 바로 추가할 수 있으므로 메모리를 절약할 수 있음
        // 따라서, + 를 이용해서 문자열을 결합하는 것보다는 StringBuilder를 이용하는 것이 더 나음
        StringBuilder sb = new StringBuilder();
        sb.append("Hello, ").append("World").append("!!");
        System.out.println(sb.toString());  // sunmin_뒤에 꼭 toString을 붙여줘야함
