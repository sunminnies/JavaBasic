...
class FinalSungJuk extends MidSungJuk {
    ...
    public FinalSungJuk(String name, int kor, int eng, int mat, int soc, int sci) {
        super(name, kor, eng, mat); // 부모클래스의 생성자를 이용해서 기본변수들을 초기화함
    ...
    public void computeSungJuk() {
        ...
        // 부모클래스의 멤버변수 접근지정자가 private으로 선언된 경우 원칙적으로 접근 불가
        // 따라서, setter/getter를 이용해서 코드 완성 => 코드가 다소 지저분해지는 단점 존재

        tot = kor + eng + mat + soc + sci;
        // 부모클래스의 멤버변수 접근지정자가 protected으로 선언된 경우 접근 가능
    ...
    // 기능별로 모듈화를 적용하는 경우 상속관계에서 메서드 재정의 시 편리하게 코드를 작성할 수 있음
    @Override
    public void getTotal() {
        super.getTotal();
        tot = tot + soc + sci;
