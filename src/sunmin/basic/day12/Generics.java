...
public class Generics {
    public static void main(String[] args) {
        // 제네릭Generic
        // 다양한 데이터 타입의 객체를 다루는 메서드나 컬렉션 클래스에 컴파일할 때 타입체크(자료형검사)를 해주는 기능
        // 즉, 클래스에 사용할 타입의 정보를 동적으로 넘겨줄 수 있고
        // 실행시간(실행할 때)에 발생할 수 있는 타입간 오류를 컴파일 시간(컴파일할 때)에 발견할 수 있도록 해주는 기능
        // JDK 1.5에서부터 추가된 기능
        ...
        // Object형으로 배열을 선언하면 어떤 데이터타입의 객체든지간에 모두 저장 가능
        Object[] zoo1 = new Object[3];
        zoo1[0] = lion1;
        zoo1[1] = tiger1;
        zoo1[2] = zebra1;

        // 단, 배열에서 객체를 빼낼때는 그 데이터타입에 맞는 적절한 형변환이 필요함
        ((Lion)zoo1[0]).sayName();
        ((Tiger)zoo1[1]).sayName();
        ((Zebra)zoo1[2]).sayName();

        // 배열에서 꺼낸 객체를 적절히 변환하려니 귀찮다. 그냥 하나의 타입으로 일괄적으로 변환하자!
...
//        ((Tiger)zoo1[2]).sayName();
        ...
        // JDK 1.5 이전에서는 여러 타입을 사용하는 대부분의 클래스나 메서드에서 인수나 반환값으로 Object 타입을 사용했었음
        // 이런 경우, 반환된 Object 객체를 다시 원하는 타입으로 변환해야하고 운이 나쁜 경우 실행시 오류가 발생할 수도 있었음
        ...
        // 컴파일할 때는 아무 오류 없다가 실행할 때 비로소 오류가 출력됨

        // JDK 1.5에서 도입한 제네릭을 활용하면 컴파일할 때 타입이 정해지므로 타입검사나 타입변환과 같은 번거로움이 사라짐
        // <>(다이아몬드) 기호 안에 타입변수를 지정해서 사용
        ...
        // nums2라는 배열변수에는 String 타입 값만 저장할 수 있음
