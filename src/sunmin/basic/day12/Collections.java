...
public class Collections {
    public static void main(String[] args) {
        ...
        // 주요 인터페이스
        // List : 자료 입력 순서 중요O, 중복 요소 허용O
        // Set : 자료 입력 순서 중요X, 중복 요소 허용X
        // Map : 키와 값으로 구성된 자료구조, 검색속도가 빠름

        // ArrayList
        // List 인터페이스를 구현한 클래스
        // 크기가 가변적으로 변하는 선형(순차/linear) 리스트(동적배열)
        // 즉, 리스트의 저장용량이 꽉차면 자동으로 용량이 늘어난다는 특징이 있음
        List<Integer> nums = new ArrayList<>();
        nums.add(123);  // add : 데이터 추가
        nums.add(456);
        nums.add(789);
        nums.add(1, 000);   // 특정위치에 요소 추가 (sunmin_index는 0부터 시작하므로 두번째 위치에 000 추가)

        System.out.println(nums.size());    // 리스트의 크기 출력
        System.out.println(nums.get(1));    // get : 지정한 위치의 요소를 추출

        // 리스트의 모든 요소 출력 1 - for문 사용
        ...
        for (Member member : members) {
            ...
            // toString()을 정의했기 때문에 member의 참조값이 출력되는 것이 아니고 미리 정의한 결과값이 출력됨
        }
