...
public class SungJukServiceV4Impl implements SungJukServiceV4 {
    ...
    public void showOneSungJuk() {
        ...
        String result = String.format(fmt, sj.getSjno(), sj.getName(), sj.getKor(), sj.getEng(), sj.getMat(),
    ...
    public void modifySungJuk() {
        ...
        try {
            ...
            System.out.println("입력 순서는 '이름 국어 영어 수학' 입니다");
            String name = sc.next();
    ...
    public void removeSungJuk() {
        Scanner sc = new Scanner(System.in);
        try {
            System.out.print("삭제할 학생의 번호는? ");
            int sjno = Integer.parseInt(sc.next());

            int cnt = sjdao.deleteSungJuk(sjno);
            if (cnt > 0) System.out.println("성적데이터 삭제 완료!");
        } catch (Exception ex) {
            System.out.println("잘못 입력하셨습니다");
            return;
        }
