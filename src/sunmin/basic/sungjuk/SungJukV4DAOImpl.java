...
public class SungJukV4DAOImpl implements SungJukV4DAO {
    ...
    private String insertSQL = "insert into sungjuk(name, kor, eng, mat, tot, mean, grd) values (?,?,?,?,?,?,?)";
    private String selectSQL = "select sjno, name, kor, eng, mat, mid(regdate,1,10) regdate from sungjuk order by sjno";
    private String selectOneSQL = "select * from sungjuk where sjno = ?";
    private String updateSQL = " update sungjuk set name = ?, kor = ?, eng = ?, mat = ?, tot = ?, mean = ?, grd = ?, " +
                                " regdate = current_timestamp where sjno = ? ";
    ...
    // sungjuk 테이블에서 '번호, 이름, 국어, 영어, 수학, 등록일' 등을 조회한 후 ArrayList에 담아서 Service로 넘김
    @Override
    ...
    // Service에서 넘겨준 학생번호로 sungjuk 테이블을 조회한 후 조회된 모든 결과를 SungJukVO에 담아서 Service로 넘김
    @Override
    ...
    // Service에서 넘겨준 학생번호로 sungjuk 테이블을 조회한 후 수정할 데이터로 sungjuk 테이블을 수정함
    @Override
    ...
    // Service에서 넘겨준 학생번호로 sungjuk 테이블을 조회한 후 해당 성적데이터를 삭제함
    @Override
