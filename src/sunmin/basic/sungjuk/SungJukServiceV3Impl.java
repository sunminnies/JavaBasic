...
public class SungJukServiceV3Impl implements SungJukServiceV3 {
    ...
    // JSON 직렬화를 위한 문자열 포맷
    String jsonfmt = "{\"name\":\"%s\",\"kor\":%d,\"eng\":%d,\"mat\":%d,\"tot\":%d,\"avg\":%.1f,\"grd\":\"%s\"},\n";
    ...
    public void newSungJuk() {
        ...
        try {
            ...
            // 방금 입력된 성적데이터와 기존에 입력된 성적데이터 모두를 SungJuk.data 파일에 직렬화해서 저장함
            // saveSungJuk();
            // sunmin_한번 입력할때마다 기존 데이터와 새로운 데이터를 함께 저장하므로 성능상 안좋음
            // sunmin_종료시 한번만 저장하면 오류 시 데이터가 날라갈 위험성은 있지만 성능상으로는 유리함
            // sunmin_ data warehouse : 창고(적재) -write 요구: OLTP
            // data lake : 소비(사용) - read 요구: OLAP
    ...
    // 저장된 성적데이터들 중에서 배열인덱스, 이름, 국어, 영어, 수학만 뽑아서 목록(list) 형태로 출력
    @Override
    ...
    public void showOneSungJuk() {
        ...
        try {
            ...
            System.out.printf(fmt, pos, sj.getName(), sj.getKor(), sj.getEng(), sj.getMat(), sj.getTot(), sj.getAvg(), sj.getGrd());
        } catch (Exception ex) {
    ...
    // 기존에 입력된 성적데이터를 역직렬화로 파일에서 읽어서 메모리에 적재하는 기능 담당
    @Override
    ...
    // 메모리 내에 저장된 모든 성적데이터를 직렬화를 통해 파일에 저장
    @Override
    ...
    // 기존에 입력된 성적데이터를 csv역직렬화로 파일에서 읽어서 메모리에 적재하는 기능 담당
    @Override
    public void initSungJuk2() {
        ...
        try (
            ...
            while(br.ready()) {
                // csv파일에서 문자열 한줄을 읽어서 쉼표단위로 분리한 후 배열로 생성
                String line = br.readLine();
                String data[] = line.split(",");

                // 생성된 배열을 이용해서 성적 객체를 만들어
                // 총점, 평균 학점을 구한 다음 (만든 생성자에 따른 옵션)
                // ArrayList에 저장함
    ...
    // 메모리 내에 저장된 모든 성적데이터를 csv직렬화를 통해 파일에 저장
    @Override
    public void saveSungJuk2() {

        // JDK 7부터는 자원을 쉽게 해제할 수 있도록 try-with-resources 라는 구문을 지원함
        try (
            ...
            for(SungJukVO sj : sjdata) {
                ...
                // csvfmt에 따라 혜교,54,76,98,265,99.3,수 형식의 데이터가 저장됨
                bw.write(result);
            }
        } catch (Exception ex) {
            ex.printStackTrace();
    ...
    // 기존에 입력된 성적데이터를 json역직렬화로 파일에서 읽어서 메모리에 적재하는 기능 담당
    @Override
    public void initSungJuk3() {
        ...
        try (
            ...
            // 텍스트 파일에 저장된 반정형데이터를 JSON 분석기(parser)를 이용해서 메모리에 객체형태로 저장함
            ...
            // 반복문을 이용해서 배열객체의 각 요소를 하나씩 꺼내 성적객체에 담고 ArrayList에 저장함
    ...
    // 메모리 내에 저장된 모든 성적데이터를 json직렬화를 통해 파일에 저장
    @Override
        ...
        try (
            ...
            for(SungJukVO sj : sjdata) {
                result = String.format(jsonfmt, sj.getName(), sj.getKor(), sj.getEng(), sj.getMat(),
                        sj.getTot(), sj.getAvg(), sj.getGrd());
                // {"name":"혜교", "kor":54, "eng":76, "mat":98, "tot":265, "avg":99.3, "grd":"수"}, 형식의 데이터가 저장됨
                bw.write(result);
            }
            bw.write("] }"); // json 배열 끝
        } catch (Exception ex) {
